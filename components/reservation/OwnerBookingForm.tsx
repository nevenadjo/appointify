...
export default function ManualReservationForm({
  ...
  async function handleSubmit(event: React.SubmitEvent<HTMLFormElement>) {
    ...
    if (!/^([01]\d|2[0-3]):[0-5]\d$/.test(startTime))
      next.startTime = "Please select a valid start time.";
    if (!/^([01]\d|2[0-3]):[0-5]\d$/.test(endTime))
      next.endTime = "Please select a valid end time.";
    const start = new Date(date + "T" + startTime);
    const end = new Date(date + "T" + endTime);
    if (!Object.keys(next).length)
      Object.assign(
        next,
        validateReservationTimes(start.toISOString(), end.toISOString()),
      );
    setErrors(next);
    ...
    try {
      ...
      setError("Unable to block this time. Please try again.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <>
      <button
        ...
        className="mt-4 min-h-11 cursor-pointer rounded-xl bg-gray-900 px-5 py-3 text-white hover:bg-gray-800"
      >
        Block time
      </button>
      {open && (
        <Modal titleId={id} onClose={() => setOpen(false)} pending={loading}>
          <h2 id={id} className="pr-12 text-xl font-semibold">
            Block time
          </h2>
          <FormErrors.Provider value={errors}>
            ...
            >
              <p className="text-sm bg-[#E9EAFF] p-4 rounded-lg">
                Block time to make a specific time period unavailable for client
                bookings, for example during breaks, personal appointments, or
                other commitments.
              </p>
              <div>
                <label
              ...
              <button
                ...
                className="ml-3 min-h-11 rounded-xl cursor-pointer border px-5 py-3 hover:bg-gray-100"
              >
                Cancel
              </button>
            </form>
